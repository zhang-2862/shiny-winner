#include "KeyRecomonder.hh"
#include "Dictionary.hh"

static const int kNumberOfCandidateWords = 6; //候选词数量

KeyRecomonder::KeyRecomonder(string& sought,
...
size_t KeyRecomonder::nBytesCode(const char ch) {
    if(ch & (1 << 7))
    {
        int nBytes = 1;
...
std::size_t KeyRecomonder::length(const std::string &str) {
   std::size_t ilen = 0;
...
int KeyRecomonder::triple_min(const int &a, const int &b, const int &c) {
   return a < b ? (a < c ? a : c) : (b < c ? b : c);
}

int KeyRecomonder::editDistance(const std::string & lhs, const std::string &rhs) {
   //计算最小编辑距离-包括处理中英文
